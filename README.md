...
- Automatic connection calibration after peer verification: the actual sender measures an 8 MiB one-way WebRTC sample, combines it with RTT, device capability, and the negotiated SCTP message-size limit, then tunes chunk size, read-ahead, disk batching, the browser-owned send queue, and a receiver-advertised in-flight window. Payload chunks scale only when the measured path and negotiated connection support it.
- The high-throughput transfer path reads multi-megabyte blocks from the sender, sends negotiated DataChannel chunks through a tightly bounded 2–8 MiB browser queue, batches receiver disk writes, checkpoints persistent resume state less aggressively, and keeps hashing/storage work in workers where supported. A separate receiver-credit window is capped at 32 MiB and self-tunes from committed write/hash speed, so BlinkSend can keep a LAN busy without hiding tens of megabytes inside browser queues. Sender progress, speed, and ETA are driven by bytes the receiver has actually committed.
...
- Large files stream to disk instead of accumulating in page memory. On OPFS-capable browsers, BlinkSend uses a dedicated receive worker. It prefers `FileSystemSyncAccessHandle`; when that is unavailable but worker-side `createWritable()` exists, writes still stay off the page main thread. Receive batches are transferred without first being merged/copied on the UI thread, and assembly, storage I/O, and receive-side SHA-256 remain in the worker. A bounded memory download remains the fallback where disk streaming is unavailable.
