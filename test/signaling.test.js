...
test('serves the app and pairs only two browsers, forwarding signaling and peer departure', async () => {
  ...
  try {
    ...
    const firstJoined = await firstJoin; assert.equal(firstJoined.type, 'joined'); assert.equal(firstJoined.count, 1); assert.match(firstJoined.iceToken, /^[A-Za-z0-9_-]+$/); assert.match(firstJoined.pairCode, /^[A-Z2-9]{8}$/); assert.ok(firstJoined.pairCodeExpires>Date.now());
    ...
    const metrics=await fetch(`http://127.0.0.1:${port}/metrics`,{headers:{authorization:'Bearer metrics-test'}});assert.equal(metrics.status,200);const metricsText=await metrics.text();assert.match(metricsText,/blinksend_rooms 1/);assert.match(metricsText,/blinksend_pair_resolves_total 1/);delete process.env.METRICS_TOKEN;
