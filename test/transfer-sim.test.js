...
async function security(){const source=await readFile(new URL('../public/security.js',import.meta.url),'utf8');const ctx={window:{},TextEncoder};vm.createContext(ctx);vm.runInContext(source,ctx);return ctx.window.BlinkSecurity;}
...
test('duplicate chunk marks are idempotent and invalid ranges are rejected',async()=>{const p=await protocol(),s=await security(),map=new Uint8Array(1);p.markChunk(map,2);p.markChunk(map,2);assert.deepEqual([...map],[4]);assert.equal(s.validRanges([[2,4],[4,5]],10),false);assert.equal(s.validRanges([[2,4],[6,9]],10),true);});
...
test('relative path sanitizer blocks traversal',async()=>{const s=await security();assert.equal(s.safeRelativePath('Photos/2026/a.jpg'),'Photos/2026/a.jpg');assert.equal(s.safeRelativePath('../secret.txt'),null);assert.equal(s.safeRelativePath('a/../../b'),null);});
