(() => {
  ...
  function makeReceiveState(size,chunkSize,receivedMap=null,nextChunk=0,received=0){
    const totalChunks=BlinkSecurity.chunkCount(size,chunkSize);if(!Number.isFinite(totalChunks)||totalChunks>BlinkSecurity.LIMITS.maxChunks)return null;
  ...
  function commitChunk(state,seq,bytes){
    ...
    BlinkProtocol.markChunk(state.receivedMap,seq);state.received+=bytes;while(state.nextChunk<state.totalChunks&&bitmapHas(state.receivedMap,state.nextChunk))state.nextChunk++;return {...check,committed:true};
  }
  function missingRanges(state){return BlinkProtocol.missingRanges(state.receivedMap,state.totalChunks);}
