(() => {
  ...
  async function makePeer() {
    ...
    try { const response = await fetch('/ice'); if (response.ok) iceServers = (await response.json()).iceServers; } catch { /* Default STUN is sufficient for many networks. */ }
  ...
  async function control(raw) {
    ...
    if (msg.type === 'cancel') { if (pending?.id === msg.id) { pending = null; els.incoming.hidden = true; status(readyLabel, true); } if (active?.id === msg.id) stopTransfer('Transfer cancelled by other device.', false); }
